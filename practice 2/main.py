...
districtDeputyNumbers = [14, 5, 7, 5, 4, 3, 29, 13, 2, 2, 8, 8, 2, 4, 2, 2, 3, 4, 3, 3, 16, 4, 3, 5, 7, 10, 3, 4, 5, 3,
                         7, 6, 10, 5, 2, 3, 10, 2, 5, 70, 24, 8, 3, 3, 3, 4, 8, 4, 3, 3, 2, 9, 16, 6, 7, 10, 6, 12, 6,
                         4, 3, 3, 7, 4, 3, 6, 9, 3, 3, 6, 11, 3, 5, 7, 8, 2, 3, 7, 2, 6, 5]
...
city_code = int(input("Please enter an index between 0 and 80 (both are inclusive):"))
while city_code < 0 or city_code > 80:
    print("You entered an invalid input!!!")
    city_code = int(input("Please enter an index between 0 and 80 (both are inclusive):"))



def vote_calcutator(city_code):

    votes = voteNumbersOfParties[city_code]
    seatnum = districtDeputyNumbers[city_code]
    seat_party = [0] * len(votes)

    for _ in range(seatnum):
        max_score = -1
        max_index = -1
        for i in range(len(votes)):
            score = votes[i] / (seat_party[i] + 1)
            if score > max_score:
                max_score = score
                max_index = i
        seat_party[max_index] += 1


    print(f"Results for {electoralDistricts[city_code]} as follows:")

    for i in range(len(partyNames)):
        print(partyNames[i], ":", seat_party[i])

vote_calcutator(city_code)
